...
class Student {
    public:
    //default constructor - public access 
    Student(){
        cout<< "calling student default construtor"<<endl;
    }
    
    //copy constructor - const ref
    // call object rhs (value) into current object
...
int main() 
{
    ...
    if(true) {
        ...
        cout<< "s1 address in main: " << &s1 << std::endl;
        cout<< "s2 address in main: " << &s2 << std::endl;
        ...
        total = getStudentGPA2(&s1, &s2); // get the memory address `&`
